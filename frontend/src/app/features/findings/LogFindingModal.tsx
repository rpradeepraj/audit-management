...
import { FindingSeverity, EvidenceAttachment } from "../../shared/types/audit";
...
export const LogFindingModal: React.FC<LogFindingModalProps> = ({
  ...
  const { audits, customers, currentUser, addFinding, showSuccess, showError } = useAudit();
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!title.trim() || !description.trim()) {
      showError("Please fill in finding title and description.");
      return;
    }

    if (!currentAudit || !currentCustomer) {
      showError("Please select a valid audit engagement.");
      return;
    }
    ...
    try {
      ...
      showSuccess(`Finding "${title}" logged successfully for audit ${currentAudit.auditNumber}.`);
      onClose();
    } catch (err: any) {
      showError(err.message || "Failed to log non-conformity finding.");
    }
  };
