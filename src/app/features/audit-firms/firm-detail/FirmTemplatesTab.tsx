...
export const FirmTemplatesTab: React.FC<FirmTemplatesTabProps> = ({
  ...
  const { firms, addTemplate, toggleFirmTemplate } = useAudit();
                  ...
                  return (
                    <tr key={t.id} className="hover:bg-slate-50/70 transition-colors">
                      ...
                      <td className="py-3 px-4 text-right">
                        <button
                          onClick={() => onToggleTemplate(firm.id, t.id)}
                          className="px-3 py-1.5 rounded-xl text-xs font-bold inline-flex items-center gap-1.5 bg-rose-50 text-rose-700 hover:bg-rose-100 border border-rose-200 transition-colors cursor-pointer"
                          title="Unlink template from firm"
                        >
