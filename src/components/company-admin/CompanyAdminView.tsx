...
import { User, UserRole, AuditFirm, AuditTemplate } from "../../types/audit";
import {
  Building2,
  Users,
  Plus,
...
import { FirmListView } from "./FirmListView";
...
export const CompanyAdminView: React.FC = () => {
  ...
  const handleSaveFirm = (
    ...
    if (editingId) {
      updateFirm(editingId, firmData);
    } else {
      const newFirmId = addFirm(firmData);
      setSelectedFirmId(newFirmId);
    }
  };

  // Delete Firm
  const handleDeleteFirm = (firmId: string, firmName: string, e?: React.MouseEvent) => {
    if (e) e.stopPropagation();
    if (firms.length <= 1) {
  ...
  const firmStaffMembers = users.filter(
  ...
  return (
    <div className={isExecutingAudit ? "w-full p-0 space-y-0" : detailedFirmId ? "w-full p-6 space-y-0" : "w-full p-4 sm:p-6 lg:p-8 space-y-6"}>
      {/* Top Header */}
      {!isFullScreenView && (
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 text-indigo-600 text-xs font-bold uppercase tracking-wider mb-1">
              ...
              <span>Audit Firm Management</span>
            </div>
            <h1 className="text-xl sm:text-2xl font-black text-slate-900">
              Audit Firm Operations & Governance
            </h1>
          ...
          {/* Top Action Button removed - placed on tab right end */}
        </div>
      )}
      ...
      */}
        ...
        <div className="flex items-center justify-between border-b border-slate-200 gap-4">
          <div className="flex items-center gap-2 overflow-x-auto">
            <button
              ...
              className={`pb-3 px-3.5 text-xs font-bold transition-all flex items-center gap-2 cursor-pointer border-b-2 whitespace-nowrap ${
                activeSubTab === "firms"
          ...
          {activeSubTab === "firms" && !detailedFirmId && (
            <button
              ...
              className="hidden sm:flex items-center gap-1.5 px-3.5 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold rounded-xl shadow-xs shadow-indigo-200 transition-colors cursor-pointer mb-2 shrink-0"
            >
          ...
          {activeSubTab === "planning" && (
            <button
              onClick={() => {
                window.dispatchEvent(new CustomEvent("open-create-audit-plan"));
              }}
              className="hidden sm:flex items-center gap-1.5 px-3.5 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold rounded-xl shadow-xs shadow-indigo-200 transition-colors cursor-pointer mb-2 shrink-0"
            >
              <Plus className="w-4 h-4" />
              <span>Schedule New Audit Plan</span>
            </button>
          )}
        </div>
      )}
        ...
        <>
          ...
          {detailedFirmId ? (
            ...
            <div className="space-y-4">
                ...
                <div className="flex items-center gap-3 w-full sm:w-auto justify-between sm:justify-end">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <select
                      ...
                      className="px-3 py-2 text-xs bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:border-indigo-500 font-medium"
                    >
              ...
              {/* Render Grid or Table View based on toggle */}
              {firmViewMode === "grid" ? (
                <FirmGridView
                  firms={filteredFirms}
                  users={users}
      ...
      {/* ========================================================================= */}
      {/* TAB 2: TEMPLATE FIRM (Maintained Compliance Frameworks) */}
      {/* ========================================================================= */}
      {activeSubTab === "template-firm" && (
        <div className="space-y-4">
          {/* Template Toolbar: Single Unified Row with Search, View Mode (Table/Grid), and Firm Selector */}
          <div className="bg-white p-3 sm:p-4 rounded-2xl border border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-3 shadow-xs">
            <div className="relative w-full sm:w-80">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
              <input
                type="text"
            ...
            <div className="flex items-center gap-3 w-full sm:w-auto justify-between sm:justify-end">
              ...
              <div className="flex items-center gap-2 shrink-0">
                <span className="text-xs font-bold text-slate-600 whitespace-nowrap">Firm:</span>
                <select
                  value={selectedFirm.id}
                  onChange={(e) => setSelectedFirmId(e.target.value)}
                  className="px-3 py-2 text-xs bg-slate-50 border border-slate-200 rounded-xl font-bold text-slate-800 focus:outline-none focus:border-indigo-500 focus:bg-white shadow-2xs cursor-pointer"
                >
                  {firms.map((f) => (
              ...
              {/* Table / Grid Switcher on Right End */}
              <div className="flex items-center bg-slate-100 p-1 rounded-xl border border-slate-200 shrink-0">
                <button
            ...
            <div className="flex items-center justify-between pb-3.5 border-b border-slate-100">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-0.5">
                    Assign auditor, manager, or administrative staff under accredited audit firm governance.
                  </p>
                </div>
              </div>
              <button
            ...
            <form onSubmit={handleSaveUser} className="space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-3 py-2 text-xs bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:border-indigo-500 focus:bg-white font-medium text-slate-900 cursor-pointer"
                >
                  {firms.map((f) => (
                    <option key={f.id} value={f.id}>
                      {f.name} ({f.code})
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g. elena.vance@pharmabioglobal.ch"
                    className="w-full px-3 py-2 text-xs bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:border-indigo-500 focus:bg-white"
                  />
                </div>
              </div>
              ...
              <div className="pt-3 border-t border-slate-100 flex items-center justify-end gap-2.5">
                ...
                >
                  <CheckCircle2 className="w-3.5 h-3.5" />
                  <span>{editingUserId ? "Update Member" : "Add Staff to Firm"}</span>
                </button>
              </div>
            </form>
          </div>
